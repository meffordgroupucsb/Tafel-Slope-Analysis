"""
qssa.py -- steady-state microkinetics by the quasi-steady-state approximation.

Define an electrocatalytic mechanism in the MECHANISM block below, then run:
    python qssa.py

The QSSA sets every coverage derivative to zero and solves the algebraic system
N . v(theta) = 0 (with the site balance) directly by a root find, instead of
integrating in time. For a serial mechanism this is exact and reproduces the
numerical steady state; it is the fast workhorse. No rate-limiting-step
assumption is made. The scan uses continuation (each overpotential starts from
the previous solution) for robustness.
...
# output controls
CSV_OUT   = "qssa.csv"        # CSV filename to write, or None to skip
...
def residual(free, eta):
    """Steady-state residual: the independent coverage derivatives a root find drives to zero.
    ...
    Returns:
        The independent d(theta)/dt values (all zero at the QSSA steady state).
    """
    d = Nmat @ net_rates(full_theta(free), eta)
    return np.delete(d, si)                      # drop the site row; it is slaved by the balance


def steady_state(eta, seed=None):
    """Solve N . v = 0 algebraically (the QSSA steady state).

    Args:
        eta:  overpotential (V).
        seed: initial guess for the independent coverages; defaults to a clean surface.
              The eta-scan passes the previous solution here (continuation).

    Returns:
        (j, theta, free): current density, full coverage vector, and the independent
        coverages (the last is reused as the seed for the next overpotential).
    """
    y0 = np.zeros(len(SPECIES) - 1) if seed is None else seed     # clean surface, or the previous solution
    sol = root(lambda y: residual(y, eta), y0, method="hybr", tol=1e-13)   # algebraic root find
    theta = np.clip(full_theta(sol.x), 0.0, 1.0)                  # solution, clipped to [0, 1]
    return current(theta, eta), theta, np.delete(theta, si)


def tafel_slope(eta, seed=None, deta=0.005):
    """Local Tafel slope (mV/dec) at eta from a central finite difference of log|j|.

    Args:
        eta:  overpotential (V).
        seed: initial-guess coverages passed through to steady_state.
        deta: half-width of the finite-difference window (V).

    Returns:
        Tafel slope (mV/dec, positive magnitude); inf if the current is flat.
    """
    jm = abs(steady_state(eta - deta, seed)[0]); jp = abs(steady_state(eta + deta, seed)[0])
...
if __name__ == "__main__":
    ...
    print(f"# QSSA (algebraic root find) -- {', '.join(st['label'] for st in STEPS)}")
    print(f"{'eta/V':>7}{'j/A cm^-2':>14}   {head}   {'b/mV dec^-1':>11}")
    rows = []
    seed = None
    for eta in ETAS:                                         # scan; reuse the previous solution as the seed
        j, th, seed = steady_state(eta, seed)               # continuation for robust convergence
        b = tafel_slope(eta, seed)
    ...
    if SHOW_PLOT:
        show_polarization([r[0] for r in rows], [r[1] for r in rows], title="QSSA")
